...
class WikiSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if (len(html_text.split('body{behavior:url("/w/static/1.27.0-wmf.7/skins/Vector/csshover.min.htc")}')) == 2):
            edited_text = html_text.split('body{behavior:url("/w/static/1.27.0-wmf.7/skins/Vector/csshover.min.htc")}')[1]
        elif (len(html_text.split('body{behavior:url("/w/static/1.27.0-wmf.6/skins/Vector/csshover.min.htc")}')) == 2):
        ...
        name = re.sub('/', ' ', name)
        edited_text = edited_text.split('Navigation menu')[0]
        if(self.search_type.strip() == 'DOMAIN'):
            filename = 'Domain_Docs/' + self.dirname.strip() + '/' + name + '.txt'
        ...
        for sel in response.xpath('//p'):
            if(len(sel.xpath('a/@href').extract()) >= 1):
                for link in sel.xpath('a/@href').extract():
                    if(len(link)>0):
                        if(('.wikipedia.org' in link) or ('wiki/Special:' in link) or ('wiki/Template_talk:' in link) or ('wiki/Template:' in link) or ('wiki/Wikipedia:' in link) or ('wiki/Category:' in link) or ('wiki/Portal:' in link) or ('wiki/Main_Page:' in link)):
                            continue
                        if('/wiki/' in link):
